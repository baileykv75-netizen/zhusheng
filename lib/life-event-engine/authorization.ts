...
export function evaluateAuthorization(
  ...
  const threshold = notBefore ? Date.parse(notBefore) : null;
  const matching = records.filter((record) =>
    record.eventId === eventId
    && record.action === proposal.action
    && record.targetBusinessId === proposal.targetBusinessId
    && (threshold === null || Date.parse(record.decidedAt) >= threshold)
  );
  if (!matching.length) return { status: "PENDING", record: null, reason: "Waiting for a resident or property authorization record for the current request" };
  ...
  return { status: "APPROVED", record, reason: "Matching human authorization was approved for the current request" };
}
