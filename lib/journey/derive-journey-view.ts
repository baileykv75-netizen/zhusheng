...
export function deriveLifeEventJourney(state: LifeEventState): JourneyView {
  switch (state) {
    ...
    case "ACTION_PROPOSED":
      return view("DIAGNOSIS", "查看联合诊断", "诊断来自建筑记忆、拓扑和透明规则；规则分值不是故障概率。", focusAction("查看诊断依据", "/property", "REVIEW_DIAGNOSIS", "diagnosis"), "诊断已形成", state);
    case "AUTHORIZATION_PENDING":
      return view("AUTHORIZATION", "等待人工授权", "阀门保持开启。只有住户完成当前事件授权后，物业才能执行模拟关阀。", focusAction("提交人工授权", "/resident?focus=authorization", "REQUEST_AUTHORIZATION", "authorization"), "等待住户授权", state);
    case "AUTHORIZED":
      return view("AUTHORIZATION", "授权已记录，等待执行", "人工批准不会自动改变阀门；仍需物业明确执行模拟关阀。", focusAction("模拟执行关阀", "/property", "EXECUTE_ISOLATION", "execute-valve"), "已授权，尚未执行", state);
    case "SIMULATED_ACTION_APPLIED":
    case "VERIFYING":
      return view("AUTHORIZATION", "验证关阀后的变化", "提交关阀后的新湿度和微流量观察，旧证据不能作为隔离后结果。", focusAction("提交隔离后观察", "/property", "VERIFY_ISOLATION", "isolation"), "正在验证关阀后的变化", state);
    case "ISOLATION_CONFIRMED":
    case "REPAIR_PENDING":
      return view("REPAIR", "事件已控制，等待维修", "隔离有效只说明供水链得到支持，不等于已经维修或解决。", focusAction("填写维修记录", "/property", "RECORD_REPAIR", "repair"), "事件已临时控制", state);
    case "REPAIR_RECORDED":
      return view("REPAIR", "维修已记录，等待恢复供水", "开阀需要一份独立人工授权，关阀授权不能复用。", focusAction("确认恢复供水授权", "/resident?focus=authorization", "REQUEST_REOPEN_AUTHORIZATION", "authorization"), "等待恢复供水授权", state);
    case "POST_REPAIR_VERIFYING":
      return view("REPAIR", "验证维修后的真实结果", "只有恢复供水后的新观察持续正常，事件才能进入已解决状态。", focusAction("提交维修后观察", "/property", "VERIFY_REPAIR", "post-repair"), "正在维修后复验", state);
    case "RESOLVED":
    ...
    case "REOPENED":
      return view("DIAGNOSIS", "异常仍存在，需要重新诊断", "维修后观察没有恢复正常，事件已重新打开，不会被错误关闭。", focusAction("重新检查1602", "/property", "REASSESS_EVENT", "evidence"), "事件已重新打开", state);
  }
}
...
function deriveDemoJourney(input: Extract<JourneyInput, { source: "DEMO" }>): JourneyView {
  ...
  if (snapshot.currentStep <= 4) {
    return view("DIAGNOSIS", "调用建造记忆联合诊断", "异常观察与隐蔽工程记录被重新连接，系统会明确证据缺口。", routeAction(snapshot.currentStep === 3 ? "调取建造记忆" : "查看物业任务", snapshot.currentStep === 3 ? "/resident" : "/property", "OPEN_EVENT_TASK"), "正在联合诊断", `DEMO_STEP_${snapshot.currentStep}`);
  }
  if (snapshot.currentStep === 5 || snapshot.incident?.status === "awaiting_authorization") {
    return view("AUTHORIZATION", "等待住户授权", "设备动作已冻结；未授权时阀门保持开启。", routeAction("提交人工授权", "/resident?focus=authorization", "REQUEST_AUTHORIZATION"), "等待人工授权", snapshot.incident?.status);
  }
  if (snapshot.currentStep === 6 || snapshot.valve.status === "closed") {
    return view("REPAIR", "验证隔离并进入维修", "关阀用于临时控制，维修与复验仍需继续完成。", routeAction("查看维修与复验", "/property", "OPEN_REPAIR_TASK"), "事件已临时控制", snapshot.incident?.status);
  }
...
export function deriveJourneyView(input: JourneyInput): JourneyView {
  ...
  if (input.phase === "DECISION") {
    return view("DIAGNOSIS", "查看有来源的联合判断", "事实、推断和不确定项已分开呈现，下一步由领域状态决定。", routeAction("进入物业运行席", "/property", "OPEN_PROPERTY_TASK"), "已完成本轮评估");
  }
