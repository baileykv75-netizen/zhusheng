...
test("resident follow-up is visually separated from the initial observation intake", async () => {
  const source = await readFile(finishingUrl, "utf8");

  assert.match(source, /\.resident-triage-summary/);
  assert.match(source, /\.resident-followup-reason/);
  assert.match(source, /\.resident-followup-actions/);
  assert.match(source, /\.resident-secondary/);
});
