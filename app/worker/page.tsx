...
import { LocalEvidenceUpload, type LocalEvidenceSelection } from "@/components/LocalEvidenceUpload";
...
export default function WorkerPage() {
  ...
  const [workerPhoto, setWorkerPhoto] = useState<LocalEvidenceSelection | null>(null);
  const latest = state.evidence.find((item) => item.id === "EV-2848");
  const stage = state.currentStep >= 2 ? 4 : state.workerSubstep || 1;
  const workerDraftReady = Boolean(transcript.trim() && fields.process.trim() && fields.pressure.trim() && workerPhoto);
  ...
  function submitWorkerDraft() {
    if (!workerDraftReady || !workerPhoto) return;
    confirmWorkerEvidence({ transcript, ...fields, photo: workerPhoto });
  }
      ...
      <section className={`field-recorder ${stage >= 2 ? "compact" : "drawer"}`}>
        ...
        {stage === 1 ? (
          <div className="capture-instrument">
            ...
            <div className="capture-copy"><strong>{recorded ? "口述已完成" : recording ? paused ? "口述已暂停" : `正在口述 · ${duration}` : "开始口述施工情况"}</strong><small>{recorded ? "下一步由AI整理口述，再补齐本轮影像身份" : "说明施工状态、保压结果与现场情况"}</small></div>
          ...
          <div className="structured-record">
            <div className="record-lead"><small>AI已整理 · 请人工确认</small><h2>已识别关键施工事实</h2><p>空间与构件身份来自扫码/BIM任务上下文，不作为自由文本修改；工序、保压结果与原始口述可由现场人员修正。品质核验前还必须确认本轮影像的数据身份。</p></div>
            ...
            <LocalEvidenceUpload
              label="选择本轮施工现场照片"
              help="图片字节只在本机预览；EV-2848仅保存数据身份和必要文件元数据"
              syntheticExample="/assets/v6/evidence/construction-pipe-install.webp"
              onSelectionChange={setWorkerPhoto}
              onReadyChange={(ready) => { if (!ready) setWorkerPhoto(null); }}
            />
            <EvidenceStrip ids={["joint", "pressure"]} label="脱敏参考素材 · 不是本轮上传" />
            <div className="evidence-row"><span><FileImage size={15} />本轮影像 <strong>{workerPhoto ? workerPhoto.dataClass === "BROWSER_LOCAL" ? "本地照片已确认" : "脱敏示例已确认" : "待补充"}</strong></span><span><ShieldCheck size={15} />结构化身份 <strong>2/2 已绑定</strong></span></div>
            <button className="stage-decision" disabled={!workerDraftReady} onClick={submitWorkerDraft}>确认内容与影像身份并提交品质核验</button>
            {!workerPhoto ? <p className="disabled-reason">还缺本轮影像身份。静态参考图不会自动算作本次提交；请选择本地照片，或明确使用脱敏合成示例。</p> : null}
          </div>
        ) : stage === 3 ? (
          <div className="quality-submitted">
            <span className="seal-mark"><ShieldCheck size={28} /></span><small>品质核验已提交</small><h2>记录 EV-2848 等待写入建筑记忆</h2><p>品质智能体已核对人工确认后的口述、工序、保压结果与本轮影像身份；空间和构件继续使用扫码/BIM绑定的结构化 BusinessId。图片字节仍只留在浏览器，本条记录保存的是证据身份与必要元数据。</p>
            <EvidenceStrip ids={["joint", "pressure"]} label="脱敏参考素材 · 非本轮上传" />
            <dl><div><dt>空间</dt><dd>{WORKER_SPACE_ID}</dd></div><div><dt>构件</dt><dd>{WORKER_COMPONENT_ID}</dd></div><div><dt>影像身份</dt><dd>{latest?.attachment?.dataClass ?? workerPhoto?.dataClass ?? "未记录"}</dd></div><div><dt>下一步</dt><dd>写入建筑生命记忆</dd></div></dl>
            <button className="stage-decision" onClick={next}>写入建筑记忆</button>
          </div>
        ) : (
          <div className="memory-seal compact-success">
            <span className="seal-mark"><CheckCircle2 size={30} /></span><small>BUILDING MEMORY · 已写入</small><h2>这次施工经历不会在封板后消失</h2><p>{latest?.id || "EV-2848"} 已作为当前确认的冷水施工证据留在建筑记忆视图中；空间、系统和构件使用统一 BusinessId，影像只保存数据身份与必要元数据，防水、闭水等其他记录继续保持各自来源。</p>
            <EvidenceStrip ids={["joint", "pressure"]} label="脱敏参考素材 · 非EV-2848本轮上传" />
            <dl className="memory-object"><div><dt>空间</dt><dd>{WORKER_SPACE_LABEL} · {fields.room}</dd></div><div><dt>构件</dt><dd>{WORKER_COMPONENT_LABEL} · {fields.component}</dd></div><div><dt>工序</dt><dd>{fields.process}</dd></div><div><dt>保压结果</dt><dd>{fields.pressure}</dd></div><div><dt>影像身份</dt><dd>{latest?.attachment?.dataClass ?? "未记录"}</dd></div></dl>
