...
首次运行会安装依赖并构建生产版本，随后打开 `http://127.0.0.1:4173`。结束时双击 `stop-zhusheng.cmd`。
...
公开站点不设置登录。演示状态保存在每台设备的 `sessionStorage` 中，访客之间互不影响；刷新可恢复本设备进度，重置可回到初始状态。图片只做浏览器本地预览，不上传云端。
