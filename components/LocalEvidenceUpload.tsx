...
export type LocalEvidenceSelection =
...
type Props = {
  ...
  onSelectionChange?(selection: LocalEvidenceSelection): void;
};

export function LocalEvidenceUpload({ label, help, syntheticExample, onSelected, onReadyChange, onSelectionChange }: Props) {
  ...
  function select(file?: File) {
    ...
    onSelected?.(file);
    onSelectionChange?.({ dataClass: "BROWSER_LOCAL", fileName: file.name, mediaType: file.type || "application/octet-stream", size: file.size });
    onReadyChange?.(true);
  }
  ...
  function useSynthetic() {
    ...
    onSelectionChange?.({ dataClass: "DEMO_SYNTHETIC", assetPath: syntheticExample });
    onReadyChange?.(true);
  }
  ...
  return <div className="local-evidence-upload">
    ...
    <p><Camera size={12} />图片字节仅在浏览器会话中预览，不上传服务器；提交方只记录数据身份和必要文件元数据。</p>
