...
export function BuildingAgentDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return createPortal(<div className="building-agent-shell" role="dialog" aria-modal="true" aria-label="筑生总智能体">
    ...
    <aside className="building-agent-drawer">
      <header className="agent-drawer-header"><div><span>ASK THE BUILDING</span><h2>问这栋房子</h2><p>它会先整理你的描述，经你确认后才调用建筑记忆。</p></div><div className="agent-header-actions"><em className={`agent-mode ${session.mode}`}>建筑记忆已连接</em><button ref={closeRef} onClick={onClose} aria-label="关闭"><X size={18} /></button></div></header>
      ...
      <div className="agent-body">
        ...
        {response ? <section className="agent-response"><header><div><span>03 / 这栋房子的回答</span><h3>{response.intent === "EVALUATE_EVENT" ? "我已经把事实与建筑记忆放在一起" : "我已完成信息编排"}</h3></div></header><div className="agent-answer-grid"><article><strong>已确认事实</strong>{response.facts.slice(0, 4).map((item) => <p key={item}><Check size={12} />{item}</p>)}</article><article><strong>当前判断</strong>{response.inferences.length ? response.inferences.slice(0, 2).map((item) => <p key={item}>{item}</p>) : <p>还没有足够信息形成判断</p>}</article><article><strong>还缺什么</strong>{response.uncertainties.length ? response.uncertainties.slice(0, 3).map((item) => <p key={item}>{item}</p>) : <p>当前无新增证据缺口</p>}</article></div><details className="agent-tool-trace"><summary><GitBranch size={14} />技术与审计详情</summary><div>{response.toolCalls.map((call, index) => <article key={`${call.callId}-${index}`}><i>{String(index + 1).padStart(2, "0")}</i><span><strong>{call.specialistAgent}</strong><code>{call.toolName}</code><p>{call.outputSummary}</p><small>来源 {call.sourceRefs.join(" / ") || "无外部来源"}</small></span></article>)}</div></details><div className="agent-next"><div><strong>唯一下一步</strong><p>{response.proposedNextAction?.label ?? "继续描述希望查询的建筑问题"}</p><small>{response.safetyNotice}</small></div>{response.proposedNextAction?.route ? <button onClick={() => router.push(response.proposedNextAction!.route!)}>进入对应工作台<ArrowRight size={15} /></button> : null}</div></section> : null}
      ...
      <footer className="agent-drawer-footer"><span><Database size={13} />建筑记忆</span><span><LocateFixed size={13} />16F / 1602 / 卫生间</span><span><Sparkles size={13} />语言理解可用</span><button onClick={reset}><RotateCcw size={13} />重置会话</button></footer>
    </aside>
  </div>, document.body);
}
